...
###########################  PB58  ######################################
test_mat <- read.csv("PB58_rewiring_cor.csv", header=T)
test_mat <- read.csv("wgcna_hardthreshold/PB58_rewiring_sig.csv", header=T)
edge_list <- test_mat[,c("Gene_i", "Gene_j", "score")]

rewire_mat <- matrix(0, nrow=5540, ncol=5540)
row.names(rewire_mat) <- colnames(Final_Data)[6:5545]
colnames(rewire_mat) <- colnames(Final_Data)[6:5545]

edge_list$newscore <- edge_list$score / max(edge_list$score)

for(i in 1:15343030){
  #take each row, find row that matches to gene i and column that matches gene j and add score to that cell
  g1 <- edge_list$Gene_i[i]
  g2 <- edge_list$Gene_j[i]
  rewire_mat[g1,g2] <- edge_list$newscore[i]
  rewire_mat[g2,g1] <- edge_list$newscore[i]
}


#rewire_graph <- graph.data.frame(edge_list, directed = F)
#rewire_mat <- as_adjacency_matrix(rewire_graph, type="both", names = T, sparse = F, attr="score")
test_pb <- test_mat[,-1]
rownames(test_pb) <- test_mat[,1]



powers = c(c(1:10), seq(from = 12, to=20, by=2))
sft = pickSoftThreshold.fromSimilarity(as.matrix(test_pb), powerVector = powers, verbose = 5)
...
softPower = 5
adjacency = adjacency.fromSimilarity(as.matrix(test_pb), power = softPower)
...
MEList = moduleEigengenes(test_pb, colors = dynamicColors)
...
merge = mergeCloseModules(test_pb, dynamicColors, cutHeight = MEDissThres, verbose = 3)
...
cluster_df <- data.frame(row.names(test_pb), moduleLabels, moduleColors)

write.csv(cluster_df, file='wgcna_PB58_p5/clustering_pb58.csv', row.names=F)

ordered_clusters_pb <- cluster_df[with(cluster_df, order(moduleLabels)),]
write.csv(ordered_clusters_pb, file='wgcna_PB58_p5/ordered_clusters_pb.csv', row.names=F)
...
write.csv(K13_cluster, file='wgcna_PB58_p5/K13_cluster_pb.csv', row.names=F)
...
PBMEs = MEs
#save(list=ls(), file='wgcna_PB58_p5/wgcna_pb_alone.RData')
plotTOM = dissTOM^7
diag(plotTOM) = NA
sizeGrWindow(9,9)
...
powers = c(c(1:10), seq(from = 12, to=20, by=2))
sft = pickSoftThreshold.fromSimilarity(as.matrix(test_nf), powerVector = powers, verbose = 5)
...
softPower = 6
adjacency = adjacency.fromSimilarity(as.matrix(test_nf), power = softPower)
TOM = TOMsimilarity(adjacency)
...
write.csv(cluster_df, file='wgcna_NF54_p6/clustering_nf54.csv', row.names=F)

ordered_clusters_pb <- cluster_df[with(cluster_df, order(moduleLabels)),]
write.csv(ordered_clusters_pb, file='wgcna_NF54_p6/ordered_clusters_nf.csv', row.names=F)
...
write.csv(K13_cluster, file='wgcna_NF54_p6/K13_cluster_nf.csv', row.names=F)
...
NFMEs = MEs
#save(list=ls(), file='wgcna_NF54_p6/wgcna_p6_nf.RData')
plotTOM = dissTOM^12
diag(plotTOM) = NA
sizeGrWindow(9,9)
