...
set.seed(0)
cluster_res <- mcl(x=con_adj_mat, addLoops = F, inflation = 1.2, max.iter =300)
...
write.table(gc_reform$names_vec, "./clustering_output/consnet_MCL_i1.2_QL.csv", quote = F, row.names = F, col.names = F, sep = "")
...
write.csv(genes_cluster, "./clustering_output/my_format/consnet_MCL_i1.2_KM.csv", row.names = F, col.names = T, quote=F)
