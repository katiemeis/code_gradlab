#calculates modularity for a network - network gets permuted - clusters defined by GO terms
#input is edgelist with no duplicates
#output is modularity score to file 
file_list = c("rf_5000.csv", "rf_7500.csv", "rf_10000.csv","rf_25000.csv","rf_50000.csv","rf_75000.csv",
               "rf_100000.csv", "rf_150000.csv", "reg_5000.csv", "reg_7500.csv", "reg_10000.csv","reg_25000.csv","reg_50000.csv","reg_75000.csv",
               "reg_100000.csv", "reg_150000.csv", "pear_5000.csv", "pear_7500.csv", "pear_10000.csv","pear_25000.csv","pear_50000.csv","pear_75000.csv",
               "pear_100000.csv", "pear_150000.csv")
...
calc_modularity <- function(file_name){
  ...
  go_ten_index <- which(gosumsvec > 1) #1 for test
  ...
  #scramble the nodes - 
  scrambled_nodes <- sample(row.names(adj_mat))
  row.names(adj_mat) <- scrambled_nodes
  colnames(adj_mat) <- scrambled_nodes
  ...
  m=dim(final_edges)[1] * 0.5 #only edges between GO terms nodes in network
  ...
  #return(Q_val)
  write.table(Q_val, file = paste("mod_perm_",file_name, sep = ""), append=T, sep=",", row.names = F, col.names = F)
}

for(file in file_list){
  for(i in 1:1000){
    calc_modularity(file)
  }
}
